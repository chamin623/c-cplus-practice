...
/*
	...
	- 생성자는 기본 생성자와 매개변수가 있는 생성자로 분류한다.
	- 생성자가 기본 생성자일 경우 생략 가능하고, 대신 컴파일러가
	  자동으로 생성자를 생성한다.

	소멸자(필수 요소)
	- 소멸자 이름은 클래스와 동일하다. 앞에 '~'을 붙임
	- 소멸자가 정의되어 있지 않으면 컴파일러가 자동으로 기본 소멸자를
	  제공한다.
*/
//Dog 클래스 정의
class Dog {
...
int main()
{
	...
	Dog dog1; //Dog dog1()와 같고 기본 생성자로 인스턴스 생성
	dog1.type = "푸들";
	dog1.age = 2;
	dog1.showInfo();
